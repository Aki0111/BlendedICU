...
class DataPreparator(DataProcessor):
    ...
    # 数据预处理类，继承自 DataProcessor，用于处理医疗数据集中的时间序列和标签数据。
    # 参数:
    #     dataset (str): 数据集名称。
    #     col_stayid (str): 表示住院ID的列名。
	...
	def __init__(self, dataset, col_stayid):
		...
		self.inch_to_cm = 2.54	# 英寸转厘米的转换系数
		self.lbs_to_kg = 0.454	 # 磅转千克的转换系数
		self.seconds_in_a_day = 24*60*60	# 一天的秒数
		self.seconds_in_an_hour = 60*60	# 一小时的秒数

	@staticmethod
	def _get_name_as_parquet(pth):
		
        # 将给定路径的文件名转换为 .parquet 格式。
        # 参数:
        #     pth (str or Path): 原始文件路径。
        # 返回:
        #     str: 转换后的 .parquet 文件名。
	...
	def get_labels(self, lazy=False):
		if lazy:
			#self.labels_savepath =E:\Science\Blended\hirid_data\labels.parquet
	...
	def write_as_parquet(pth_src,
		...
		for i, df in enumerate(df_chunks):
			#enumerate是python的一个内置函数，用于迭代一个序列，返回一个迭代器对象。
  ...
  #                   )
		...
		def _map_itemids(lf, itemid_label, id_mapping):
			if itemid_label:
				# print(f'调试 id_mapping={id_mapping}')
				str_id_mapping = {str(k): v for k, v in id_mapping.items()}
				return lf.with_columns(
					pl.col(itemid_label).cast(pl.Utf8).replace(str_id_mapping).alias(col_variable)
					)
			return lf
